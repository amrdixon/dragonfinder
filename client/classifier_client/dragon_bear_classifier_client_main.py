    ...
    def classify_image_from_path(self, image_path):
        ...
        try:
            ...
            if x.status_code == 200:
                ...
                print('Model Predictions \n{} : {}'.format(image_path, self._class_labels[int(res_logit>=0.0)]))                
            else:
...
@click.group()
@click.option('--url', default='http://localhost:8501/v1/models/dragon_bear_classifier_mobilenetv2')
@click.option('--labels', default=['Bear', 'Dragon'])
@click.pass_context
def cli(ctx, url, labels):
    """Dragon vs. Bear Classifier Command Line Tool"""
    
    ctx.obj = ImageBinaryClassifierClient(url, labels)

@cli.command()
@click.argument('image', type=click.Path(exists=True))
@click.pass_obj
def classify_image(ctx, image):
    
    ctx.classify_image_from_path(image)
